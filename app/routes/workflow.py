...
async def update_workflow(workflow_id: int, workflow: WorkflowCreate, db: Session = Depends(get_db)):
    try:
        logger.info(f"Updating workflow {workflow_id}: {workflow.workflow_name}")
        return await workflow_service.update_workflow(db, workflow_id, workflow)
